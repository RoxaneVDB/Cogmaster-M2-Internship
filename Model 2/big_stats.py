...
def make_stats():
    ...
    for ka in kapas:
        ...
        for k in range(nbr_simus):
            dir = direction+'Kapa_' + str(ka).rjust(nbr_kapas_size,"0") + "/Simu_" + str(k).rjust(nbr_simu_size,"0") + '/'
...
def make_plot(clust_average,prop_empty,prop_quasi_empty) :
    ...
    ax.set_xlim(1,0) # changes nothing visible but keeps the thing coherent
    ax.set_ylim(0,1.1)
    ax.legend()
