...
class Population:
    def __init__(self,threshold,noise): # Random initialisation
        self.time = 0
        ...
        self.noise = noise # Proba of deleting or creating a connexion
    ...
    def low_rep_flow_friend(self,i): # returns the id of one of the friends with the lowest rep flow (randomly between the worst)
            ...
            if (not found_a_friend):
                ...
                min_trust = self.rep_flow_graph[i][k]
                l = [k] # list of equaly less trusted friends
                for j in range (n):
                    if self.coop_graph[i][j]:
                        if self.rep_flow_graph[i][j] == min_trust:
                            l.append(j)
                        if self.rep_flow_graph[i][j] < min_trust:
                            l = [j]
                            min_trust = self.rep_flow_graph[i][j]
    ...
    def actualise_coop(self):
        for i in range(number_of_people):
            for j in range(i):
                if self.rep_flow_graph[i][j] < self.threshold:
                    self.coop_graph[i][j] = 0
                    self.coop_graph[j][i] = 0
                else:
                    self.coop_graph[i][j] = 1
                    self.coop_graph[j][i] = 1
        ...
        for i in my_pop:
            ...
            if l > max_friends:
                for k in range(l-max_friends):
                    #self.actualise_utility() # -> TROP COUTEUX !!!
                    supressed_friend = self.low_rep_flow_friend(i)
                    self.coop_graph[i][supressed_friend] = 0
                    self.coop_graph[supressed_friend][i] = 0
    
    """
    def limit_friends(self):
    #suppress connexions if someone has more friends than the max authorised by deleting RANDOM friends
        my_pop = [i for i in range(number_of_people)]
        random.shuffle(my_pop)
        for i in my_pop:
            friends = self.friends_list(i)
            ...
            if l > max_friends:
                for k in range(l-max_friends):
                    supressed_friend = friends.pop(0)
                    self.coop_graph[i][supressed_friend] = 0
                    self.coop_graph[supressed_friend][i] = 0
    """
        ...
        for i in range(number_of_people):
            for j in range(i):
                if random.random() < self.noise:
                    coop = self.coop_graph[i][j]
                    self.coop_graph[i][j] = 1 - coop
                    self.coop_graph[j][i] = 1 - coop
